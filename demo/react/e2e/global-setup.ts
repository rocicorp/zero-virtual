...
// The stack (docker compose, .env, zero-cache-dev) lives in demo/shared.
...
function spawnZeroCache(port: number) {
  const binDir = join(SHARED_DIR, 'node_modules', '.bin');
  ...
  const bin = join(SHARED_DIR, 'node_modules', '.bin', 'zero-cache-dev');
