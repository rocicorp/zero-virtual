...
describe('firstVisibleItem / lastVisibleItem', () => {
  test('report the edges of the viewport, not of the loaded window', () => {
    // 400px viewport, 20px rows: rows 0-19 are in view, and the loaded window
    // is much larger than that.
    const h = harness({rowCount: 500});
    h.settle();

    expect(h.core.firstVisibleItem()?.index).toBe(0);
    expect(h.core.lastVisibleItem()?.index).toBe(19);
    expect(h.core.getSnapshot().items.length).toBeGreaterThan(20);
  });

  test('follow the scroll position', () => {
    const h = harness({rowCount: 500});
    h.settle();

    h.userScroll(500); // 500 / 20 = row 25
    h.tick();

    expect(h.core.firstVisibleItem()?.index).toBe(25);
    expect(h.core.lastVisibleItem()?.index).toBe(44);
  });

  test('count a row that overlaps the scrollport at all', () => {
    // The same test paging uses: a row half out of view is still visible.
    const h = harness({rowCount: 500});
    h.settle();

    h.userScroll(510); // half of row 25 is above the top edge
    h.tick();

    expect(h.core.firstVisibleItem()?.index).toBe(25);
  });

  test('carry the row and its key, which scrollToItem takes', () => {
    const h = harness({rowCount: 500});
    h.settle();
    h.userScroll(500);
    h.tick();

    const first = h.core.firstVisibleItem()!;
    expect(first.row).toEqual({id: 'r25'});

    // The round trip keyboard navigation makes: ask where you are, then jump.
    h.core.scrollToItem(first.key, {align: 'start'});

    expect(h.rowTop('r25')).toBe(0);
  });

  test('are undefined with no scroll element attached', () => {
    const h = harness({rowCount: 500});
    h.settle();
    h.core.detach();

    expect(h.core.firstVisibleItem()).toBeUndefined();
    expect(h.core.lastVisibleItem()).toBeUndefined();
  });
});
...
describe('scrollToItem', () => {
  ...
  describe('addressing a rendered row by its key', () => {
    // Documented shortcut: the string is matched against the rendered rows
    // before any lookup, so keyboard navigation can pass `items[].key`.
    const keyed = {
      anchoring: 'manual' as const,
      getRowKey: (r: TestRow) => `key-${r.id}`,
    };

    test('scrolls, and issues no lookup at all', () => {
      const h = harness({rowCount: 500, options: keyed});
      h.settle();

      h.core.scrollToItem('key-r50', {align: 'start'});

      expect(h.rowTop('key-r50')).toBe(0);
      // The shortcut's whole point: no query was opened for it.
      expect(h.core.getQueryInputs().probeID).toBeNull();
    });

    test('takes a numeric key without the caller stringifying it', () => {
      // A placeholder row inside the loaded window is keyed by its index, so
      // `items[].key` is a number there and the signature has to take one.
      const h = harness({rowCount: 500, options: {anchoring: 'manual'}});
      h.settle();
      const item = h.core.getSnapshot().items.find(i => i.index === 50)!;

      h.core.scrollToItem(item.key, {align: 'start'});

      expect(h.rowTop(String(item.key))).toBe(0);
    });

    test('reaches rendered rows only: an off-screen key does nothing', () => {
      // The documented limit. `key-r400` is not rendered, so it falls through
      // to the lookup, which reads it as an id and resolves nothing.
      const h = harness({rowCount: 500, options: keyed});
      h.settle();
      const firstBefore = h.core.getSnapshot().items[0].index;

      h.core.scrollToItem('key-r400', {align: 'start'});
      h.settle();
